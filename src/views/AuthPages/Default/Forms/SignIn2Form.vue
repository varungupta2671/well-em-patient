<template>
  <ValidationObserver ref="form" v-slot="{ handleSubmit }">
    <form novalidate @submit.prevent="handleSubmit(onSubmit)"> <!--- class="mt-4" --->
      <p>Enter your aadhar id to access your account.</p>
      <ValidationProvider vid="aadharid" name="Aadhar Id" rules="required" v-slot="{ errors }">
        <div class="form-group">
          <label for="hcareInput">Aadhar Id</label>
          <input
            type="text"
            :class="'form-control mb-0' +(errors.length > 0 ? ' is-invalid' : '')"
            id="hcareInput"
            aria-describedby="emailHelp"
            v-model="user.aadharid"
